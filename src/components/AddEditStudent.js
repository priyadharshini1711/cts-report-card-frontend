...
export default function AddEditStudent({ toggleAddEditModal, mode, initialValues }) {

  const handleSubmit = (e) => {
    values.role = values.role === 'Member' ? 2 : 1
    if(mode === MODE.CREATE) {
      addStudent(values)
    } else {
      editStudent(values)
              ...
              <div className="flex-1 items-start">
                <div className="mt-3 text-left ml-4">
                  <Dialog.Title as="h3" className="text-lg leading-6 font-medium text-gray-900">
                    {mode === MODE.CREATE ? 'Add Student' : 'Edit Student'}
                  </Dialog.Title>
                  <div className="mt-2">
                    <form onSubmit={onSubmit}>
                      ...
                      <div className="mt-3">
                        ...
                        <select
                          id="role"
                          name="role"
                          className="h-8 mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"
                          defaultValue="Member"
                          value={values.role}
                          onChange={handleChange}
                        >
