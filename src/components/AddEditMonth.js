...
import { addMonth, editMonth } from '../services/month'
import { MODE } from '../constants'

export default function AddEditMonth({ toggleAddEditModal, mode, initialValues }) {

  const handleSubmit = (e) => {
    values.name = values.name.toUpperCase()
    values.key = values.name.substring(0,3)
    if(mode === MODE.CREATE) {
      addMonth(values)
    } else {
      editMonth(values)
              ...
              <div className="flex-1 items-start">
                <div className="mt-3 text-left ml-4">
                  <Dialog.Title as="h3" className="text-lg leading-6 font-medium text-gray-900">
                    {mode === MODE.CREATE ? 'Add Month' : 'Edit Month'}
                  </Dialog.Title>
                  <div className="mt-2">
